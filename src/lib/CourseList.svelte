<script>
    ...
    function getTotalCredit(selectedCourseNames, curSemesterData) {
      ...
      return selectedCourseNames
        ...
        .map((c) => {
          ...
          if (
            !Object.prototype.hasOwnProperty.call(curSemesterData[c], "credits")
          ) {
            return 0;
          }
    ...
    function getUnique(courseName) {
      return [...new Set($curSemesterData[courseName].rooms)];
    }
  </script>
  ...
  >
    ...
    <div
      class="divide-y dark:divide-zinc-500"
      on:mouseleave={() => hoveredCourse.set("")}
    >
      {#if $curSemesterData && $selectedCourseNames && $selectedCourseNames.length > 0}
        {#each $selectedCourseNames as courseName}
          ...
          >
            ...
            <ul class="ml-2">
              {#each getUnique(courseName) as room}
                <li>{room}</li>
              {/each}
            </ul>
          </div>
